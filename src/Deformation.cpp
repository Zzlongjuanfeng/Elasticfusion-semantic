...
//                                const u_char & label_src,
//                                const u_char & label_target,
                                const uint64_t & srcTime,
                                const uint64_t & targetTime,
                                const bool pinConstraints)
{
    //Add the new constraint
    constraints.push_back(Constraint(src.head(3), target.head(3)/*, label_src, label_target*/, srcTime, targetTime, false));

    if(pinConstraints)
    {
        constraints.push_back(Constraint(target.head(3), target.head(3)/*, label_src, label_target*/, targetTime, targetTime, false, true));
    }
}
                            ...
                            std::vector<Constraint> * newRelativeCons)
{
    if(def.isInit())
    {
        ...
        for(size_t i = 0; i < constraints.size(); i++)
        {
            if(constraints.at(i).relative)
            {
                def.addRelativeConstraint(constraints.at(i).srcPointPoolId, constraints.at(i).tarPointPoolId/*, constraints.at(i).srcLabel, constraints.at(i).targetLabel*/);
            }
            else
            {
                Eigen::Vector3f targetPoint = constraints.at(i).target;
                def.addConstraint(constraints.at(i).srcPointPoolId, targetPoint/*, constraints.at(i).srcLabel, constraints.at(i).targetLabel*/);
            }
        }
...
//                                                              constraints.at(i).srcLabel,
//                                                              constraints.at(i).targetLabel,
