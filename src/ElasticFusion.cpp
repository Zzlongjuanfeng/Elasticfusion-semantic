...
void ElasticFusion::computeFeedbackBuffers()
{
    ...
    feedbackBuffers[FeedbackBuffer::RAW]->compute(textures[GPUTexture::RGB]->texture,
                                                  textures[GPUTexture::DEPTH_METRIC]->texture,
                                                  textures[GPUTexture::LABEL]->texture,
                                                  tick,
                                                  maxDepthProcessed);
...
bool ElasticFusion::denseEnough(const Img<Eigen::Matrix<unsigned char, 4, 1>> & img)
{
    int sum = 0;
...
void ElasticFusion::processFrame(const int count_global,
                                 ...
                                 const bool bootstrap)
{
    ...
    textures[GPUTexture::LABEL]->texture->Upload(label, GL_RGB, GL_UNSIGNED_BYTE);
        ...
        if(closeLoops && ferns.lastClosest != -1)
        {
            ...
            else
            {
                for(size_t i = 0; i < constraints.size(); i++)
                {
                    ...
                    float * vertices = new float[globalModel.bufferSize];

                    memset(&vertices[0], 0, globalModel.bufferSize);

                    glGenTransformFeedbacks(1, &globalModel.vbos[0].second);
                    glGenBuffers(1, &globalModel.vbos[0].first);
                    glBindBuffer(GL_ARRAY_BUFFER, globalModel.vbos[0].first);
                    glBufferData(GL_ARRAY_BUFFER, globalModel.bufferSize, &vertices[0], GL_STREAM_DRAW);
                    glBindBuffer(GL_ARRAY_BUFFER, 0);
//                    globalModel.model();//vbos[target]

                    globalDeformation.addConstraint(constraints.at(i).sourcePoint,
                                                    constraints.at(i).targetPoint,
//                                                    constraints.at(i).sourceSurfaceLabel,
//                                                    constraints.at(i).targetSurfaceLabel,
                                                    tick,
...
//                localDeformation.constrain(ferns.frames, rawGraph, tick, false, poseGraph, false, &newRelativeCons))
...
//}}
//                std::cout<<consBuff.cols<<std::endl;
                ...
                for(int i = 0; i < consBuff.cols; i++)
                {
                    for(int j = 0; j < consBuff.rows; j++)
                    {
                           ...
                           timesBuff.at<unsigned short>(j, i) > 0)
                        {
                            ...
                            unsigned char labelSrc=label[j*640*3+i*3];
//                            unsigned char labelTarget=vertices[];

//                            std::cout<<labelSrc<<std::endl;
//                            std::cout<<imageBuff.at<Eigen::Matrix<unsigned char, 4, 1>>(j, i)(3)<<std::endl;
                            constraints.push_back(Ferns::SurfaceConstraint(worldRawPoint, worldModelPoint/*,labelSrc,imageBuff.at<Eigen::Matrix<unsigned char, 4, 1>>(j, i)(3)*/));

                            localDeformation.addConstraint(worldRawPoint,
                                                           worldModelPoint,
//                                                           labelSrc,
//                                                           imageBuff.at<Eigen::Matrix<unsigned char, 4, 1>>(j, i)(3),
                                                           tick,
        ...
        if(!rgbOnly && trackingOk && !lost)
        {
            ...
            globalModel.clean(count_global,
                              flag_slam,
                              currPose,
                              tick,
                              indexMap.indexTex(),
