...
void main()
{
    ...
    vec4 label_ = textureLod(lSampler, texcoord.xy, 0.0);

//if ((label_.x*255.0f==0.0f))//||(vColor.x*255.0f==50.0f)||(vColor.x*255.0f==90.0f)||(vColor.x*255.0f==150.0f)||(vColor.x*255.0f==160.0f))
//{
    
    vColor = textureLod(cSampler, texcoord.xy, 0.0);
if (flagslam==1)
{
    vColor.x = encodeColor1(vec4(vColor.xyz, label_.x));
}
else
{
    vColor.x = encodeColor0(vec4(vColor.xyz, label_.x));
}
    vColor.y = 0;
