...
//       initNorms(norms)
    ...
    for(int i = 0; i < num; i++)
    {
        ...
        if(verts.at<Eigen::Vector4f>(conservatory.at(i).pos(1), conservatory.at(i).pos(0))(2) > 0)//z (depth)
        {
            const Eigen::Matrix<unsigned char, 4, 1> & pix = img.at<Eigen::Matrix<unsigned char, 4, 1>>(conservatory.at(i).pos(1), conservatory.at(i).pos(0));
                                 ...
                                 const bool lost)
{
    ...
    for(int i = 0; i < num; i++)
    {
        ...
        {
            const Eigen::Matrix<unsigned char, 4, 1> & pix = imgSmall.at<Eigen::Matrix<unsigned char, 4, 1>>(conservatory.at(i).pos(1), conservatory.at(i).pos(0));
...
//        std::cout<<"frames.at(i)->srcTime:   "<<frames.at(i)->srcTime<<std::endl;
        ...
        if(rgbd.lastICPError < 0.0003 && rgbd.lastICPCount > icpCountThresh && photoError < photoThresh)
        {
            ...
            for(int i = 0; i < num; i += num / 50)
            {
                   ...
                   int(vertSmall.at<Eigen::Vector4f>(conservatory.at(i).pos(1), conservatory.at(i).pos(0))(2) * 1000.0f) < maxDepth)
                {
                    ...
                    cv::Mat imagelabel(480,640,CV_8UC3);//,cv::Scalar(0,0,0));
                    int m=0;
                    for (int i=0;i<480;i++)
                    {
                        for (int j=0;j<640;j++)
                        {
                            imagelabel.at<unsigned int>(i*640*3+j)=label[m++];
                        }
                    }
//                    cv::imshow("a",imagelabel);
//                    cv::waitKey(0);
                    constraints.push_back(SurfaceConstraint(worldRawPoint, worldModelPoint/*, label[conservatory.at(i).pos(1)*width*3+conservatory.at(i).pos(0)*3], imgSmall.at<Eigen::Matrix<unsigned char, 4, 1>>(conservatory.at(i).pos(1), conservatory.at(i).pos(0))(3)*/));
                }
            }
        }
    }
                              ...
                              const unsigned char * fernRgb)
{
    ...
    Img<Eigen::Matrix<unsigned char, 4, 1>> imgFern(height, width, (Eigen::Matrix<unsigned char, 4, 1> *)fernRgb);
    ...
    for(int i = 0; i < num; i++)
    {
           ...
           int(vertSmall.at<Eigen::Vector4f>(conservatory.at(i).pos(1), conservatory.at(i).pos(0))(2) * 1000.0f) < maxDepth)
        {
            ...
            if(correspondence(0) >= 0 && correspondence(1) >= 0 && correspondence(0) < width && correspondence(1) < height &&
               (imgFern.at<Eigen::Matrix<unsigned char, 4, 1>>(correspondence(1), correspondence(0))(0) > 0 ||
                imgFern.at<Eigen::Matrix<unsigned char, 4, 1>>(correspondence(1), correspondence(0))(1) > 0 ||
                imgFern.at<Eigen::Matrix<unsigned char, 4, 1>>(correspondence(1), correspondence(0))(2) > 0))
            {
                photoSum += abs((int)imgFern.at<Eigen::Matrix<unsigned char, 4, 1>>(correspondence(1), correspondence(0))(0) - (int)imgSmall.at<Eigen::Matrix<unsigned char, 4, 1>>(conservatory.at(i).pos(1), conservatory.at(i).pos(0))(0));
                photoSum += abs((int)imgFern.at<Eigen::Matrix<unsigned char, 4, 1>>(correspondence(1), correspondence(0))(1) - (int)imgSmall.at<Eigen::Matrix<unsigned char, 4, 1>>(conservatory.at(i).pos(1), conservatory.at(i).pos(0))(1));
                photoSum += abs((int)imgFern.at<Eigen::Matrix<unsigned char, 4, 1>>(correspondence(1), correspondence(0))(2) - (int)imgSmall.at<Eigen::Matrix<unsigned char, 4, 1>>(conservatory.at(i).pos(1), conservatory.at(i).pos(0))(2));
                photoCount++;
            }
        }
    }
