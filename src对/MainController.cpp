...
void MainController::launch(cv::Mat &image_in, cv::Mat &depth_in, cv::Mat &labelImg/*, Eigen::Matrix4f tran*/, const bool flag_slam, int64_t count_global)
{
//    while(good)
//    {
        if(eFusion)
        {
            run(image_in, depth_in, labelImg/*, tran*/, flag_slam, count_global);
        }
...
void MainController::run(cv::Mat &image_in, cv::Mat &depth_in, cv::Mat &labelImg/*, Eigen::Matrix4f& tran*/, const bool flag_slam, int64_t count_global)
{
...
//                tran=tran.inverse();
                eFusion->processFrame(logReader->rgb, logReader->depth/*, logReader->label_ef*/, logReader->timestamp, currentPose/*&tran*/, weightMultiplier);
