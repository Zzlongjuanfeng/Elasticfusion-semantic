...
void main()
{
    ...
    //New unstable point
// unstable point 没有做 deform，unstable point 是新捕获图像中的点，这些点的位置在 fuse 阶段是用回环检测后更新的相机的位姿计算的，这里不需要再做 deform
    if(vColor.w == -2)
    {
        vColor.w = time;
    }
    ...
    //在回环检测之后会更新相机的位姿，在 fuse 阶段，新捕获的图像的位置是用更新的相机的位姿计算的
    // test == 1 模型点是 inactive 而且找不到对应点的时候
    if(test == 1 && nodes > 0 && vColor.z != time)
    {
		...
		//if (flagslam==1)
		//{
    		//   vec4 colmm = decodeColor(vCol.x);
    		//   vColor.x = transformColorLabel(colmm);
		//}
